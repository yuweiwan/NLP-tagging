...
def test_list(file, VocWords):
    ...
    for pair in s:
        ...
        if word not in VocWords:
            ...
            word_L.append(word)
        origin_L.append(word)
        tag_L.append(tag)
    return origin_L, word_L, tag_L
    ...
    for i in range(length - 1, 0, -1):
        ...
        for now_tag in tag_dict[word_list[i]]:
            ...
            for next_tag in tag_dict[word_list[i - 1]]:
                p_tt = math.log((count_tt[(next_tag, now_tag)] + smooth) / (count_t[next_tag] + smooth * Vt))
                p_tw = math.log((count_tw[(word_list[i], now_tag)] + smooth) / (count_t[now_tag] + smooth * Vw))
                p = p_tt + p_tw
...
def cal_perplexity(smooth, Vw, Vt, word_list, test_tag, count_t, count_tt, count_tw):
    ...
    length = len(test_tag)
    for i in range(1, length):
        p_tt = math.log((count_tt[(test_tag[i - 1], test_tag[i])] + smooth) / (count_t[test_tag[i - 1]] + smooth * Vt))
        p_tw = math.log((count_tw[(word_list[i], test_tag[i])] + smooth) / (count_t[test_tag[i]] + smooth * Vw))
        p += p_tt + p_tw
...
'''def divide(file):
    with open('entrain', 'r') as f:
        s = f.readlines()
    f1 = open('1', 'w')
    for i in range(0, 49997):
        f1.write(s[i])
    f1.close()
    f2 = open('2', 'w')
    f2.write('###/###')
    for i in range(49997, 99988):
        f2.write(s[i])
    f2.close()'''
...
def main():
    ...
    for i in range(len(predictP)):
        # number
        if originWord[i][0].isdigit() == True and originWord[i][-1].isdigit() == True:
            predictP[i] = 'C'
        # noun
        if originWord[i][-4:] == 'tion':
            predictP[i] = 'N'
        # 's
        if predictP[i - 1] == 'N' and originWord[i] == '\'s':
            predictP[i] = 'P'
        # OOV
        if originWord[i] not in VocWords:
            if '-' in originWord[i]:
                predictP[i] = 'J'
            elif originWord[i][-2:] == 'ed' or originWord[i][-3:] == 'ing':
                predictP[i] = 'V'
            elif originWord[i][0].isupper():
                predictP[i] = 'N'
        # ,. tag
        if predictP[i] in [',', '.'] and originWord[i] not in [',', '.']:
            if originWord[i][-3:] == 'ing' or originWord[i][-2:] == 'ed':
                predictP[i] = 'V'
            else:
                predictP[i] = 'N'
        # V
        if originWord[i] in VocWords and originWord[i][-2:] == 'ed':
            predictP[i] = 'V'
    ...
    for i in range(len(predictP)):
        f.write(originWord[i] + '/' + predictP[i] + '\n')
    f.close()
